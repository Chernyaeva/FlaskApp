...
def article_detals(article_id):
    article = Article.query.filter_by(id=article_id).options(
        joinedload(Article.tags) # load joined tags!
        ).one_or_none()
    if article is None:
        raise NotFound
...
def create_article():
    ...
    # add availible tags to form
    form.tags.choices = [(tag.id, tag.name) for tag in Tag.query.order_by("name")]
    if request.method == "POST" and form.validate_on_submit():        
        article = Article(title=form.title.data.strip(), body=form.body.data)
        if form.tags.data: # if any tags were selected in form
            selected_tags = Tag.query.filter(Tag.id.in_(form.tags.data))
            for tag in selected_tags:
                article.tags.append(tag) # add selected tags to article
